...
var require_core = __commonJS({
  "node_modules/compress-json/dist/core.js"(exports) {
    ...
    function compress(o) {
      ...
      const root2 = (0, memory_1.addValue)(mem, o, void 0);
    ...
    function decompress2(c) {
      const [values, root2] = c;
      return decode(values, root2);
    }
  }
});
...
var cosmeticChanger_default = new class CosmeticChanger {
...
var root_3 = from_html(`<h2> </h2> <!>`, 1);
var root_1 = from_html(`<input type="text" placeholder="Skin ID"/> <!>`, 1);
var root_8 = from_html(`<button> </button>`);
var root_9 = from_html(`<input type="text" placeholder="Trail ID"/>`);
var root = from_html(`<h1>Skin</h1> <div><select><option>Unchanged</option><option>Any skin by ID</option><option>Custom</option></select> <!></div> <h1>Trail</h1> <div><select><option>Unchanged</option><option>Any trail by ID</option></select> <!></div>`, 1);
var $$css = {
  hash: "svelte-1rz4ed",
  code: ".colors.svelte-1rz4ed {display:flex;flex-wrap:wrap;gap:10px;width:100%;padding:10px;}.color.svelte-1rz4ed {width:50px;height:50px;border:none;}.color.selected.svelte-1rz4ed {outline:4px solid black;}"
};
function UI($$anchor, $$props) {
  push($$props, true);
  append_styles($$anchor, $$css);
  let skinType = state(proxy(cosmeticChanger_default.skinType));
  let skinId = state(proxy(cosmeticChanger_default.skinId));
  let trailType = state(proxy(cosmeticChanger_default.trailType));
  let trailId = state(proxy(cosmeticChanger_default.trailId));
  ...
  async function onSkinIdEntered() {
    set(styles, null);
    if (!get(skinId)) return;
    let url = `https://www.gimkit.com/assets/map/characters/spine/${get(skinId)}.json`;
    ...
    set(styles, skinData.style, true);
    for (let style of get(styles).categories) {
      if (style.type === "color") {
  ...
  function save() {
    cosmeticChanger_default.setSkin(get(skinType), get(skinId), get(customSkinFile), selectedStyles);
    cosmeticChanger_default.setTrail(get(trailType), get(trailId));
  }
  ...
  var node = sibling(select, 2);
  {
    var consequent_2 = ($$anchor2) => {
      ...
      var node_1 = sibling(input_1, 2);
      {
        var consequent_1 = ($$anchor3) => {
          ...
          each(node_2, 17, () => get(styles).categories, index, ($$anchor4, category) => {
            ...
            var node_3 = sibling(h2, 2);
            {
              ...
              var alternate = ($$anchor5) => {
                var div_1 = root_5();
                each(div_1, 21, () => get(category).options, index, ($$anchor6, option, i) => {
                  var button = root_6();
                  let classes;
                  button.__click = () => selectedStyles[get(category).name] = get(option).name;
                  let styles_1;
                  template_effect(() => {
                    classes = set_class(button, 1, "color svelte-1rz4ed", null, classes, {
                      selected: selectedStyles[get(category).name] ? selectedStyles[get(category).name] === get(option).name : i === 0
                    });
                    set_attribute(button, "title", get(option).name);
                    styles_1 = set_style(button, "", styles_1, { "background-color": get(option).preview.color });
                  });
              ...
              if_export(node_3, ($$render) => {
                if (get(category).type === "color") $$render(consequent);
                else $$render(alternate, false);
              });
            }
            template_effect(() => set_text(text, get(category).name));
    ...
    var alternate_1 = ($$anchor2) => {
      ...
      var node_4 = first_child(fragment_4);
      {
        var consequent_3 = ($$anchor3) => {
          var button_1 = root_8();
          button_1.__click = uploadSkin;
          var text_1 = child(button_1);
          reset(button_1);
          template_effect(() => set_text(text_1, `Current: ${(get(customSkinFile) ? get(customSkinFile).name : "None") ?? ""}. Upload skin`));
          append($$anchor3, button_1);
        };
        if_export(
          node_4,
          ($$render) => {
            if (get(skinType) === "custom") $$render(consequent_3);
          },
          true
        );
      }
    ...
    if_export(node, ($$render) => {
      if (get(skinType) === "id") $$render(consequent_2);
      else $$render(alternate_1, false);
    });
  }
  reset(div);
  var div_2 = sibling(div, 4);
  var select_1 = child(div_2);
  var option_4 = child(select_1);
  option_4.value = option_4.__value = "default";
  var option_5 = sibling(option_4);
  option_5.value = option_5.__value = "id";
  reset(select_1);
  ...
  reset(div_2);
  bind_select_value(select, () => get(skinType), ($$value) => set(skinType, $$value));
  bind_select_value(select_1, () => get(trailType), ($$value) => set(trailType, $$value));
