...
var PI = Math.PI;
var lo = {
  initialForceFactor: 3e-3,
  maximumForce: 10,
  maximumTickForce: 3,
  forceTickMultiplier: 0.89,
  minimumForce: 0.01,
  platformerGrounded: {
    leftAngle: 7 * PI / 6,
    rightAngle: 11 * PI / 6,
...
var gi = { tickRate: 12 };
var hc = { normal: 310 };
var ce = { worldScale: 100 };
var q5 = Math.round(we.jump.cooldownMS / (1e3 / gi.tickRate));
var Y5 = Math.round(we.jump.coyoteJumpLimitMS / (1e3 / gi.tickRate));
var se = {
  width: 64,
  height: 64
};

// plugins/DownpatchPhysics/src/index.ts
...
api.net.onLoad(() => {
  ...
  api.patcher.instead(api.stores.phaser.mainCharacter.physics, "preUpdate", (thisVal) => {
    ...
    }, thisVal.tickInput = scene.inputManager.getPhysicsInput(), thisVal.currentPacketId += 1, sK({
      characterId: thisVal.character.id,
      input: thisVal.tickInput
    });
  });
  const airSpeedMinimum = 0.75;
  const originalCalcVelocity = function(A, t) {
    var e = 0, i = 0, n = null == t ? void 0 : t.angle, s = null !== n && (n < 90 || n > 270) ? "right" : null !== n && n > 90 && n < 270 ? "left" : "none", C = api.stores.me.movementSpeed / hc.normal, l = api.platformerPhysics.platformerGroundSpeed * C;
    ...
    var c = 0 !== h;
    if (s !== A.physics.state.movement.direction && (c && 0 !== A.physics.state.movement.xVelocity && (A.physics.state.movement.xVelocity = 0), A.physics.state.movement.accelerationTicks = 0, A.physics.state.movement.direction = s), A.physics.state.movement.xVelocity !== h) {
      A.physics.state.movement.accelerationTicks += 1;
      var u = 0;
      u = A.physics.state.grounded ? c ? api.platformerPhysics.movement.ground.accelerationSpeed : api.platformerPhysics.movement.ground.decelerationSpeed : c ? api.platformerPhysics.movement.air.accelerationSpeed : api.platformerPhysics.movement.air.decelerationSpeed;
      var B = 20 / gi.tickRate;
      u *= A.physics.state.movement.accelerationTicks * B, e = h > A.physics.state.movement.xVelocity ? Phaser.Math.Clamp(A.physics.state.movement.xVelocity + u, A.physics.state.movement.xVelocity, h) : Phaser.Math.Clamp(A.physics.state.movement.xVelocity - u, h, A.physics.state.movement.xVelocity);
    } else e = h;
    return A.physics.state.grounded && A.physics.state.velocity.y > api.platformerPhysics.platformerGroundSpeed * C && Math.sign(e) === Math.sign(A.physics.state.velocity.x) && (e = A.physics.state.velocity.x), A.physics.state.movement.xVelocity = e, A.physics.state.gravity = $5(A.id), i += A.physics.state.gravity, A.physics.state.forces.forEach(function(A2) {
      var n2 = A2.ticks[0];
  ...
  const k = api.stores;
  const WA = (id) => scene.characterManager.characters.get(id);
  ...
  const Si = (n) => n * ce.worldScale;
  const Zg = (n) => n / ce.worldScale;
  ...
  function sK(g) {
    ...
    let e = Fe() && !wr(), I = t.physics.state.grounded;
    g.reconciliation || (i9({
      characterId: t.id
    }), X5({
      characterId: t.id
    ...
    if (t.physics.state.velocity = s ?? {
      ...
      if (e) {
        ...
        let { headCollisions: E, groundCollisions: n, slidingCollisions: Q, numOfAllCollisions: l } = B, c = U5({
          characterId: t.id,
          groundCollisions: n,
  ...
  function i9(g) {
    ...
    let e = WA(t);
    e && e.physics.lastClassDeviceActivationId !== k.me.classDesigner.lastClassDeviceActivationId && (e.physics.lastClassDeviceActivationId = k.me.classDesigner.lastClassDeviceActivationId, k.me.classDesigner.activeClassDeviceId = k.me.classDesigner.lastActivatedClassDeviceId);
  }
  function X5(g) {
    let { characterId: t } = g, e = WA(t);
  ...
  function V5(g) {
    g.physics.state.forces = g.physics.state.forces.filter((t) => {
      var e, I;
  ...
  function t9(g) {
    ...
    let I = /* @__PURE__ */ new Set(), o = Date.now();
  ...
  function Z5(g) {
    let { projectile: t, character: e } = g, I = {
      x: t.end.x,
      y: t.end.y
    };
    let o = Phaser.Math.Angle.Normalize(Phaser.Math.Angle.Between(t.start.x, t.start.y, I.x, I.y));
    e.physics.state.grounded && Fe() && (lo.platformerGrounded.notWhenWithin.some(([E, n]) => o > E && o < n) || (o > PI / 2 && o < 3 * PI / 2 ? o = lo.platformerGrounded.leftAngle : o = lo.platformerGrounded.rightAngle));
  ...
  function lJ(g) {
    ...
    hJ({
      forceId: t.id,
      character: e
    }), t.type && (e.physics.state.forces = e.physics.state.forces.filter((I) => I.type !== t.type)), e.physics.state.forces.push(t);
  }
  function hJ(g) {
    let { forceId: t, character: e } = g;
    e.physics.state.forces = e.physics.state.forces.filter((I) => I.id !== t);
  }
  function Iy(g) {
    let { character: t } = g, e = t.physics.getBody();
    if (!e || !e.character) {
      return;
    }
    ...
    let o = /* @__PURE__ */ new Set(), s = Lr(t.id), { bodies: r } = Ve();
    Ve().world.intersectionsWithShape(e.rigidBody.translation(), 0, new RAPIER.Capsule(s.height, s.radius), (B) => {
      var l;
      let E = B.parent().userData, n = E == null ? void 0 : E.id, Q = r.find(n);
      return Q != null && Q.character || (l = Q == null ? void 0 : Q.terrain) != null && l.removed || n && (g.reset || I.has(n)) && o.add(n), true;
    }), e.character.ignoredStaticBodies = o;
  }
  function Lr(g) {
    let t = scene.characterManager.characters.get(g), e = (t == null ? void 0 : t.scale.baseScale) ?? 1;
    return Fe() ? {
  ...
  function _5(g) {
    let { characterId: t, jumpKeyPressed: e } = g, I = WA(t);
    if (!I || (I.physics.state.jump.actuallyJumped = false, I.physics.state.jump.jumpTicks += 1, e === void 0)) {
      return;
    }
    if (I.physics.state.grounded) {
      I.physics.state.jump.isJumping = false;
      let { maxJumps: r } = dJ({
        characterId: I.id
      });
      I.physics.state.jump.jumpsLeft = r, I.physics.state.jump.jumpCounter = 0;
    }
    let o = k.me.movementSpeed / hc.normal, s = we.topDownBaseSpeed * o;
    if (e && I.physics.state.jump.jumpsLeft > 0 && I.physics.state.jump.jumpTicks > q5 && s > 0) {
      let r = !I.physics.state.grounded && I.physics.state.jump.jumpCounter === 0 && I.physics.state.groundedTicks < Y5, B = v5(I, r);
      lJ({
        character: I,
        force: {
          id: "jump",
      ...
      }), I.physics.state.jump.isJumping = true, I.physics.state.jump.jumpsLeft -= 1, I.physics.state.jump.jumpTicks = 0, I.physics.state.jump.actuallyJumped = true, r && (I.physics.state.jump.jumpsLeft += 1), I.physics.state.jump.jumpCounter += 1, I.physics.state.jump.xVelocityAtJumpStart = Math.abs(I.physics.state.velocity.x), I.isMain;
      return;
    }
  }
  function dJ(g) {
  ...
  function v5(g, t) {
    ...
    let r = Math.round(e.jumpDurationMS / (1e3 / gi.tickRate)), B = T5({
      count: o,
      numTicks: r,
  ...
  function T5(g) {
    let { count: t, numTicks: e, ease: I } = g, o = [];
    let s = 0;
    for (let r = 0; r < e; r++) {
      let B = I((r + 1) / e), E = t * B - s;
      o.push(E), s += E;
    }
    return o;
  }
  ...
  function oldCalcVelocity(A, t) {
    let e = 0, i = 0;
    const s = null == t ? void 0 : t.angle, g = null !== s && (s < 90 || s > 270) ? "right" : null !== s && s > 90 && s < 270 ? "left" : "none", C = k.me.movementSpeed / hc.normal;
    ...
    return A.physics.state.grounded && A.physics.state.velocity.y > api.platformerPhysics.platformerGroundSpeed * C && Math.sign(e) === Math.sign(A.physics.state.velocity.x) && (e = A.physics.state.velocity.x), A.physics.state.movement.xVelocity = e, A.physics.state.gravity = $5(A.id), i += A.physics.state.gravity, A.physics.state.forces.forEach((A2, _t) => {
      const s2 = A2.ticks[0];
  ...
  function j5(g, t) {
    var h;
    let e = 0, I = 0;
    let o = t == null ? void 0 : t.angle, s = o !== null && (o < 90 || o > 270), r = o !== null && o > 90 && o < 270, B = s ? "right" : r ? "left" : "none", E = k.me.movementSpeed / hc.normal;
    ...
    let Q = g.physics.state.forces.find((d) => d.type === "hit");
    if (Q) {
    ...
    let c = l !== 0;
    if (B !== g.physics.state.movement.direction && (c && g.physics.state.movement.xVelocity !== 0 && (g.physics.state.movement.xVelocity = 0), g.physics.state.movement.accelerationTicks = 0, g.physics.state.movement.direction = B), g.physics.state.movement.xVelocity !== l) {
      g.physics.state.movement.accelerationTicks += 1;
      let d = 0, y = 0;
      g.physics.state.grounded ? c ? (d = we.movement.ground.accelerationSpeed, y = we.movement.ground.maxAccelerationSpeed) : d = we.movement.ground.decelerationSpeed : c ? (d = we.movement.air.accelerationSpeed, y = we.movement.air.maxAccelerationSpeed) : d = we.movement.air.decelerationSpeed;
      let N = 20 / gi.tickRate;
      d *= g.physics.state.movement.accelerationTicks * N, y && (d = Math.min(y, d)), l > g.physics.state.movement.xVelocity ? e = Phaser.Math.Clamp(g.physics.state.movement.xVelocity + d, g.physics.state.movement.xVelocity, l) : e = Phaser.Math.Clamp(g.physics.state.movement.xVelocity - d, l, g.physics.state.movement.xVelocity);
    } else {
      e = l;
    }
    return g.physics.state.grounded && g.physics.state.velocity.y > we.platformerGroundSpeed * E && Math.sign(e) === Math.sign(g.physics.state.velocity.x) && (e = g.physics.state.velocity.x), g.physics.state.movement.xVelocity = e, g.physics.state.gravity = $5(g.id), I += g.physics.state.gravity, g.physics.state.forces.forEach((d) => {
      let y = d.ticks[0];
  ...
  function H5(g) {
    let t = x5({
      characterId: g
    }), e = t.maxGravityPerSecond, I = t.timeToMaxGravityMS, o = e / gi.tickRate, s = Math.round(I / 1e3 * gi.tickRate);
    return {
  ...
  function W5(g, t) {
    ...
    if (t && t.angle !== null) {
      ...
      let r = s * o, B = Phaser.Math.DegToRad(t.angle);
      e = Math.cos(B) * r, I = Math.sin(B) * r;
    }
    return g.physics.state.forces.forEach((o) => {
      let s = o.ticks[0];
  ...
  function K5(g) {
    let { characterId: t, velocity: e } = g, I = WA(t);
    if (!I) {
      return;
    }
    ...
    let r = s.computedMovement(), E = I.physics.getBody().rigidBody.translation().y + r.y, n = F5({
      characterY: Si(E)
    });
    return r.y -= E - Zg(n), r;
  }
  function M5(g) {
    var r, B, E, n, Q;
    let { character: t, otherCollider: e } = g, I = e.parent().userData, o = I == null ? void 0 : I.id;
    if (!o) {
      return true;
    }
    let s = Ve().bodies.find(o);
    if (!s || s.character || s.sensor || (B = (r = t.physics.getBody()) == null ? void 0 : r.character) != null && B.ignoredStaticBodies.has(o) || (Q = (n = (E = t.physics.getBody()) == null ? void 0 : E.character) == null ? void 0 : n.ignoredTileBodies) != null && Q.has(o) || N5(s)) {
      return false;
    }
    if (s.device) {
  ...
  function N5(g) {
    ...
    return t ? t.parts.manuallyHiding : false;
  }
  function b5(g) {
    let t = WA(g.characterId);
    if (!t) {
    ...
    let I = e.numComputedCollisions(), o = [], s = [], r = [];
    for (let B = 0; B < I; B++) {
      let E = e.computedCollision(B), n = E == null ? void 0 : E.normal1;
      n && (n.y < we.normals.maxGroundCollision ? o.push(E) : n.y < 0 && s.push(E), n.y > we.normals.minHeadCollision && t.physics.state.jump.isJumping && Math.abs(t.physics.getBody().character.controller.computedMovement().y) < we.jump.cancelJumpIfHitHeadCausesYMovementLessThan && r.push(E));
    }
    return {
      groundCollisions: o,
      slidingCollisions: s,
  ...
  function U5(g) {
    let { characterId: t, groundCollisions: e, numOfAllCollisions: I } = g, o = WA(t);
    if (!o) {
      return false;
    }
  ...
  function L5(g) {
    ...
    if (Q) {
      let h = Q.normal, d = Phaser.Math.RadToDeg(Phaser.Math.Angle.Between(0, 0, h.x, h.y) + Math.PI / 2);
      return Math.abs(d) > we.climbAngle ? 0 : Phaser.Math.DegToRad(Math.round(d));
    }
    return 0;
  }
  function Ia(g) {
    let { start: t, end: e } = g, I = g.skipCharacters ?? false, { world: o, bodies: s } = Ve();
    ...
    let n;
    return o.intersectionsWithRay(E, 1, true, (Q) => {
      var h, d, y, N, m;
      let l = (h = Q.collider.parent().userData) == null ? void 0 : h.id, c = s.find(l);
      return I && (c != null && c.character) || c != null && c.sensor || c != null && c.device && ((y = g.ignoredDevicesIds) != null && y.includes((d = c == null ? void 0 : c.device) == null ? void 0 : d.id)) || c != null && c.device && !_i((N = c == null ? void 0 : c.device) == null ? void 0 : N.id).checkIfCollidersEnabled() || g.ignoredBodyIds && g.ignoredBodyIds.includes(l) || (m = c == null ? void 0 : c.terrain) != null && m.removed ? true : n ? (Q.toi < n.toi && (n = Q), true) : (n = Q, true);
    }, g.skipCharacters ? (
      /* NB.EXCLUDE_DYNAMIC */
      4
    ) : void 0), {
  ...
  function J5(g) {
    let { characterId: t, isGrounded: e, isHittingHead: I, groundedRotation: o, wasGrounded: s } = g, r = WA(t);
    r && (r.physics.state.groundedTicks += 1, r.physics.state.lastGroundedAngle = o ?? 0, s && !e && !r.physics.state.jump.isJumping && (r.physics.state.jump.jumpsLeft -= 1), I && r.physics.state.jump.isJumping && hJ({
      character: r,
      forceId: "jump"
    }), r.physics.state.grounded = e, r.physics.state.grounded && (r.physics.state.groundedTicks = 0));
  }
  function R5(g) {
    let { characterId: t, movement: e, velocity: I } = g, o = WA(t);
    if (!o) {
      return;
    }
    ...
    if (s) {
      if (Fe() && !wr()) {
        let r = Phaser.Math.RadToDeg(Phaser.Math.Angle.Wrap(Phaser.Math.Angle.Between(0, 0, I.x, I.y))), B = Phaser.Math.RadToDeg(Phaser.Math.Angle.Wrap(Phaser.Math.Angle.Between(0, 0, e.x, e.y))), E = Phaser.Math.Angle.ShortestBetween(r, B);
